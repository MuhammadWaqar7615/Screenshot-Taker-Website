...
// import React, { useState, useEffect, useMemo } from "react";
// import Sidebar from "../../components/Sidebar";
// import { FaEdit, FaTrash, FaCheck, FaTimes, FaPlus, FaCrown, FaUserTie } from "react-icons/fa";
// import { db } from "../../config/firebase";
// import {
//   collection,
//   onSnapshot,
...
//   const isSiteAdmin = localUser?.isSiteAdmin;
//   const isCompanyAdmin = localUser?.role?.toLowerCase() === "admin" || 
//                         localUser?.role?.toLowerCase() === "company admin";
...
//             onClick={handleAddProject}
//             disabled={saving}
//             className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded cursor-pointer disabled:opacity-50"
//           >
//             <FaPlus /> Add Project
//           </button>
//         </header>
...
//             value={clientFilter}
//             onChange={(e) => setClientFilter(e.target.value)}
//             className="border border-gray-600 px-3 py-2 rounded bg-gray-800 text-white cursor-pointer disabled:opacity-50"
//             disabled={saving}
//           >
//             <option value="">All Clients</option>
//             {uniqueClients.map((c) => (
...
//                             })
//                           }
//                           className="border rounded px-2 py-1 w-full bg-gray-800 text-white disabled:opacity-50"
//                           disabled={saving}
//                         />
//                       ) : (
//                         project.name
...
//                             })
//                           }
//                           className="border rounded px-2 py-1 w-full bg-gray-800 text-gray-300 disabled:opacity-50"
//                           disabled={saving}
//                         />
//                       ) : (
...
//                   No projects found
//                 </h3>
//                 <p className="text-gray-400">Click "Add Project" to create a new project</p>
//               </div>
//             )}
//           </div>
//         </div>
                ...
                {filteredProjects.map((project, idx) => (
                  <tr
                    ...
                    className={`${
                      ...
                      editingProject === project.id ? "border-2 border-blue-500" : ""
                    }`}
                    ...
                    <td className="px-6 py-3 text-gray-100">
                      {editingProject === project.id ? (
                        <input
                          ...
                          onClick={(e) => e.stopPropagation()}
                          onChange={(e) => setEditedData({ ...editedData, name: e.target.value })}
                          className="border border-gray-600 rounded px-2 py-1 w-full text-white bg-gray-700 disabled:opacity-50"
                          disabled={saving}
                        />
                      ) : (
                        String(getField(project, ["name", "projectName", "project_name", "title"]) || "-")
                    ...
                    <td className="px-6 py-3 text-gray-100">
                      {editingProject === project.id ? (
                        <input
                          ...
                          onClick={(e) => e.stopPropagation()}
                          onChange={(e) => setEditedData({ ...editedData, startDate: e.target.value })}
                          className="border border-gray-600 rounded px-2 py-1 w-full text-gray-300 bg-gray-700 disabled:opacity-50"
                          disabled={saving}
                        />
                      ) : (
                        formatDateVal(getField(project, ["startDate", "start_date", "start"]))
                      ...
                      {editingProject === project.id ? (
                        <>
                          <button
                            onClick={() => handleSave(project.id)}
                            disabled={!isFormValid || saving}
                            className={`${!isFormValid || saving ? "text-gray-500 cursor-not-allowed" : "text-green-400 hover:text-green-200 cursor-pointer"}`}
                          >
                            <FaCheck />
                          </button>
                          <button 
                            onClick={() => handleCancel(project.id)} 
                            className="text-red-400 hover:text-red-200 cursor-pointer disabled:opacity-50"
                            disabled={saving}
                          >
                            <FaTimes />
                          </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-[#1e293b] text-white p-8 rounded-lg shadow-lg w-[70%] max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-2 mb-4 max-h-90 overflow-y-auto">
              {selectedProject.users?.length === 0 ? (
                ...
                <table className="min-w-full border border-gray-700 rounded-lg overflow-hidden text-sm">
                  ...
                  <tbody className="bg-gray-900 text-gray-300">
                    {selectedProject.users?.map((u, idx) => (
                      <tr 
                        key={u.id} 
                        className={`${
            ...
            {!showAddForm && (
              <button 
                onClick={() => setShowAddForm(true)} 
                className="bg-green-600 hover:bg-green-700 px-4 py-2 rounded w-full cursor-pointer mb-4 disabled:opacity-50"
                disabled={saving}
              >
                + Add User
              </button>
            ...
            {showAddForm && (
              <div className="space-y-2 mb-4">
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={handleSaveUser} 
                    className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded w-full cursor-pointer disabled:opacity-50"
                    disabled={saving}
                  >
                    Save
                  </button>
                  <button 
                    onClick={() => setShowAddForm(false)} 
                    className="bg-gray-600 hover:bg-gray-700 px-4 py-2 rounded w-full cursor-pointer disabled:opacity-50"
                    disabled={saving}
                  >
                    Cancel
                  </button>
            ...
            <button 
              onClick={() => setSelectedProject(null)} 
              className="mt-4 bg-red-600 hover:bg-red-700 px-4 py-2 rounded w-full cursor-pointer disabled:opacity-50"
              disabled={saving}
            >
              Close
            </button>
