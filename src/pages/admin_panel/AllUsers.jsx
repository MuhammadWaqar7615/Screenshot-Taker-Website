...
// } from "firebase/firestore";
// import Sidebar from "../../components/Sidebar";
// import { FaEdit, FaTrash, FaTimes, FaUser, FaChevronDown, FaCrown, FaUserTie } from "react-icons/fa";
...
//   const isSiteAdmin = !!currentUser?.isSiteAdmin;
//   const isCompanyAdmin = currentUser?.role?.toLowerCase() === "admin" || 
//                         currentUser?.role?.toLowerCase() === "company admin";
//   const currentCid = currentUser?.cid || null;
...
//             <button
//               onClick={openAddModal}
//               className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg shadow cursor-pointer"
//               disabled={saving}
//             >
//               ➕ Add User
//             </button>
...
//                       onClick={() => handleRowClick(user)}
//                       title="Click to view user dateil"
...
// } from "firebase/firestore";
// import Sidebar from "../../components/Sidebar";
// import { FaEdit, FaTrash, FaTimes, FaUser, FaChevronDown, FaCrown, FaUserTie, FaSignOutAlt, FaChevronUp } from "react-icons/fa";
...
//         address: formData.address,
//         bloodGroup: formData.bloodGroup,
//         projectName: formData.projectName, // Save project name
//       };
...
//       address: user.address || "",
//       bloodGroup: user.bloodGroup || "",
//       projectName: user.projectName || "", // Set project name
//     });
//     setShowAddModal(true); // Open modal for editing
//   };
...
//       <main className="flex-1 p-6 overflow-auto">
//         {/* Top Header with User Info and Buttons */}
//         <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
...
//           <p className="text-gray-400 text-sm truncate">{currentUser?.email || "user@example.com"}</p>
//         </div>
//         <button
...
//         {/* <div className="flex display-end gap-3 mb-6">
//           <button
//             onClick={() => setShowTimerModal(true)}
//             className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg shadow cursor-pointer transition-colors"
//             disabled={saving}
//           >
//             ⏰ Set Timer
//           </button>
//           <button
//             onClick={openAddModal}
//             className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg shadow cursor-pointer transition-colors"
//             disabled={saving}
//           >
//             ➕ Add User
//           </button>
...
//                   {/* <option value="">Select Admin</option>
//                   {users
//                     .filter((u) => u.role?.toLowerCase() === "admin", "Company Admin")
//                     .map((admin) => (
//                       <option key={admin.id} value={admin.id}>
//                         {admin.name} ({getCompanyName(admin.cid)})
//                       </option>
...
// {users
//   .filter((u) => 
//     u.role && ["admin", "company admin", "super admin"].includes(u.role.toLowerCase())
//   )
//   .map((admin) => (
//     <option key={admin.id} value={admin.id}>
//       {admin.name} ({getCompanyName(admin.cid)})
...
//         {showAddModal && (
//           <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center  z-50">
//             <div className="bg-gray-800 p-8 rounded-xl shadow-xl w-full max-w-8xl  max-h-[95vh] overflow-y-auto relative ">
//               <button
//                 onClick={closeAddModal}
//                 disabled={saving}
//                 className="absolute top-3 right-3 text-gray-400 hover:text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
...
//     <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-8xl max-h-[95vh] overflow-hidden flex flex-col">
//       {/* Header */}
//       <div className="flex items-center justify-between p-6 border-b border-gray-700 bg-gray-800/50">
//         <div className="flex items-center gap-4">
//           <div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-semibold text-xl shadow-lg">
//             {selectedUser.name?.charAt(0)?.toUpperCase() || <FaUser className="text-lg" />}
//           </div>
//           <div>
//             <h2 className="text-xl font-bold text-white">{selectedUser.name || "Unnamed User"}</h2>
//             <p className="text-gray-300 text-sm">{selectedUser.email || "No email provided"}</p>
//           </div>
//         </div>
//         <button
//           onClick={closeUserModal}
//           className="w-8 h-8 rounded-full bg-gray-700 hover:bg-gray-600 flex items-center justify-center text-gray-300 hover:text-white transition-colors duration-200"
//         >
//           <FaTimes size={16} />
//         </button>
//       </div>
...
//                 </div>
//                 <div>
//                   <label className="text-sm font-medium text-gray-400 block mb-1">Contact</label>
//                   <p className="text-white text-sm">{selectedUser.contact || "—"}</p>
//                 </div>
//                 <div>
//                   <label className="text-sm font-medium text-gray-400 block mb-1">Blood Group</label>
//                   <p className="text-white text-sm">{selectedUser.bloodGroup || "—"}</p>
//                 </div>
//                 <div>
//                   <label className="text-sm font-medium text-gray-400 block mb-1">User ID</label>
//                   <p className="text-white font-mono text-sm bg-gray-900/50 px-2 py-1 rounded">
...
//               </h3>
//               <div>
//                 <label className="text-sm font-medium text-gray-400 block mb-2">Address</label>
//                 <p className="text-white text-sm leading-relaxed bg-gray-900/50 rounded-lg p-3 min-h-[60px]">
...
//                   <p className="text-white text-sm">{getCompanyName(selectedUser.cid)}</p>
//                 </div>
//                 <div>
//                   <label className="text-sm font-medium text-gray-400 block mb-1">Department</label>
//                   <p className="text-white text-sm">{selectedUser.department || "—"}</p>
//                 </div>
//                 <div>
//                   <label className="text-sm font-medium text-gray-400 block mb-1">Role</label>
//                   <p className="text-white text-sm">{selectedUser.role || "—"}</p>
//                 </div>
//                 <div>
//                   <label className="text-sm font-medium text-gray-400 block mb-1">Project Name</label>
//                   <p className="text-white text-sm">{selectedUser.projectName || "—"}</p>
//                 </div>
//                 <div>
//                   <label className="text-sm font-medium text-gray-400 block mb-1">Joining Date</label>
//                   <p className="text-white text-sm">{formatDate(selectedUser.joiningDate) || "—"}</p>
//                 </div>
//                 <div>
...
//                       selectedUser.status === "active" ? "bg-green-400" : "bg-gray-400"
//                     }`}></div>
//                     {selectedUser.status || "inactive"}
//                   </span>
//                 </div>
...
//                 <div className="space-y-4">
//                   <div>
//                     <label className="text-sm font-medium text-gray-400 block mb-1">Screenshot Timer</label>
//                     <p className="text-white font-mono text-sm">{formatTimer(selectedUser.timer)}</p>
//                   </div>
//                   <div>
...
//                 <div className="space-y-4">
//                   <div>
//                     <label className="text-sm font-medium text-gray-400 block mb-1">Created At</label>
//                     <p className="text-white text-sm">{formatDate(selectedUser.createdAt)}</p>
//                   </div>
//                   <div>
//                     <label className="text-sm font-medium text-gray-400 block mb-1">Last Updated</label>
//                     <p className="text-white text-sm">{formatDate(selectedUser.updatedAt)}</p>
//                   </div>
//                 </div>
//               </div>
...
//               </h3>
//               <div>
//                 <label className="text-sm font-medium text-gray-400 block mb-3">Skills with Experience</label>
//                 <div className="flex flex-wrap gap-2">
//                   {(selectedUser.skills && selectedUser.skills.length > 0) ? (
//                     selectedUser.skills.map((skillItem, index) => {
//                       const skill = typeof skillItem === 'object' ? skillItem.skill : skillItem;
...
//                         <span 
//                           key={index} 
//                           className="inline-flex items-center px-3 py-1.5 bg-blue-500/20 text-blue-300 text-sm rounded-lg border border-blue-500/30"
//                         >
//                           <span className="font-medium">{skill}</span>
//                           {experience > 0 && (
//                             <span className="ml-2 px-1.5 py-0.5 bg-blue-600/50 text-blue-100 text-xs rounded">
//                               {experience}y
//                             </span>
//                           )}
//                         </span>
...
//                     })
//                   ) : (
//                     <div className="text-gray-400 text-sm bg-gray-900/30 rounded-lg p-4 text-center">
//                       No skills added yet
//                     </div>
//                   )}
//                 </div>
...
//               startEdit(selectedUser, e);
//             }}
//             className="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-medium transition-colors duration-200 cursor-pointer"
//           >
//             <FaEdit size={14} />
//             Edit User
//           </button>
...
//             to={`/screenshots/${selectedUser.id}`}
//             onClick={closeUserModal}
//             className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium transition-colors duration-200 cursor-pointer text-center justify-center"
//           >
//             {/* Icon removed - no error now */}
//             View Screenshots
//           </Link>
//         </div>
//       </div>
...
//                       <div className="flex ml-115 gap-3 ">
//   <button
//     onClick={() => setShowTimerModal(true)}
//     className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg shadow cursor-pointer transition-colors"
//     disabled={saving}
...
//                     <th className="p-3 w-25 truncate whitespace-nowrap">Timer</th>
//                     <th className="p-3 w-30 truncate whitespace-nowrap">Joining Date</th>
//                     <th className="p-3 w-30 truncate whitespace-nowrap">Project Name</th>
//                     <th className="p-3 w-30 truncate whitespace-nowrap">Screenshots</th>
//                     <th className="p-3 w-28 truncate whitespace-nowrap">Actions</th>
//                   </tr>
//                 </thead>
                ...
                <tbody>
                  {filteredUsers.map((user, idx) => (
                    <tr
                      ...
                      title="Click to view user detail"
                    >
                      <td className="p-3 flex items-center gap-2 truncate whitespace-nowrap overflow-hidden">
                        <div className="w-8 h-8 rounded-full bg-gray-600 flex items-center justify-center text-white font-semibold flex-shrink-0">
                          {user.name?.charAt(0)?.toUpperCase() || "?"}
                        </div>
                        <span className="truncate">{user.name}</span>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {(user.skills && user.skills.length > 0) ? (
                            user.skills.slice(0, 2).map((skillItem, index) => {
                              ...
                              const exp = typeof skillItem === 'object' ? skillItem.experience : 0;
                              return (
                                <span key={index} className="px-1 py-0.5 bg-blue-600 text-white text-xs rounded truncate max-w-20 flex items-center gap-1">
                                  {skill}
                                  {exp > 0 && <span className="text-yellow-300">({exp}y)</span>}
                                </span>
                              );
                      ...
                      <td className="p-3 font-mono truncate whitespace-nowrap overflow-hidden">
                        {formatTimer(user.timer)}
                      </td>
                      ...
                      {/* Screenshots Link */}
                      <td className="p-3 truncate whitespace-nowrap overflow-hidden" title="View Screenshots">
                        <Link
                          to={`/screenshots/${user.id}`}
                          className="text-blue-400 hover:underline cursor-pointer"
                          onClick={(e) => e.stopPropagation()}
